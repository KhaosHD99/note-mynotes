...
#define FOLDER_NAME "folder.png"
#define FILE_NAME "file.png"
...
static gboolean load_pixbufs (GError **error)
{
	  #if 1
	  if (!file_pixbuf)
	    	file_pixbuf = gdk_pixbuf_new_from_file (FILE_NAME, error);
	    	
	  if (!folder_pixbuf)
	    	folder_pixbuf = gdk_pixbuf_new_from_file(FOLDER_NAME, error);
	  
	  if (!file_pixbuf || !folder_pixbuf)
	  		return FALSE;
	  #endif
...
static void home_clicked (GtkToolItem *item,
	      									gpointer     user_data)
{
	  GtkListStore *store;
	  store = GTK_LIST_STORE (user_data);
...
int main(int argc, char *argv[])
{
	      ...
	      if (!load_pixbufs(&error))
	      ...
	      else
		    {
				    ...
				    //tool_bar
					  ...
					  //event
					  g_signal_connect (up_button, "clicked", G_CALLBACK(up_clicked), liststore);
					  g_signal_connect (home_button, "clicked", G_CALLBACK(home_clicked), liststore);
					  ...
					  g_signal_connect (icon_view, "item_activated", 
					                    G_CALLBACK (item_activated), liststore);
   ...
   // GtkWidget *window3;
    ...
    //gtk_builder_add_from_file(builder, "iconViewBasic.glade", NULL);
	  
	  //window
	  //window = GTK_WIDGET(gtk_builder_get_object(builder, "window_main"));
	  //do_widget = (GtkWidget *)gtk_builder_get_object(builder, "window_root");
	  do_iconview ();
    //window = GTK_WIDGET(gtk_builder_get_object(builder, "window_root"));
    
		//gtk_window_move(GTK_WINDOW(do_widget), 200, 0);
		
    //event
   // gtk_builder_connect_signals(builder, NULL);
    //g_object_unref(G_OBJECT (builder));
    
    //gtk_widget_show(do_widget);
