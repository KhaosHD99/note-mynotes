          ...
          char *argv[] )
{
     ...
     * 选项或是标题栏上的关闭按钮发出的)，我们让它以上面定义的形式调用 delete_event() 函数。
     * 传给回调函数的 data 参数值是 NULL，它会被回调函数忽略。*/
    g_signal_connect (G_OBJECT (window), "delete_event",
		      G_CALLBACK (delete_event), NULL);
    
    /* 在这里我们连接 "destroy" 事件到处理函数。  
     * 调用 gtk_widget_destroy() 函数或在 "delete_event" 回调函数中返回 FALSE 值
     * 都可以触发 "destroy" 事件。*/
    ...
    /* 当收到 "clicked" 信号时会调用上面定义的 hello() 函数。*/
    g_signal_connect (G_OBJECT (button), "clicked",
		      G_CALLBACK (hello), NULL);
    
    /* 当点击按钮时，会通过调用 gtk_widget_destroy(window) 函数关闭窗口。
